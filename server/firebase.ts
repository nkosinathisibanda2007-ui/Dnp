...
import { CmsDatabase } from '../src/types';
...
try {
  ...
  if (process.env.FIREBASE_SERVICE_ACCOUNT_KEY) {
    try {
      ...
      console.warn('Failed to parse FIREBASE_SERVICE_ACCOUNT_KEY JSON:', e?.message || e);
    }
  ...
  console.warn('Firebase Admin SDK could not be initialized with service account:', err?.message || err);
}
...
// Fallback local file helper (only used in offline non-Vercel local dev)
const DATA_DIR = path.join(process.cwd(), 'data');
function readLocalFallback<T>(filename: string, fallback: T): T {
  try {
...
export async function getCmsContent(): Promise<CmsDatabase> {
  ...
  try {
    ...
    console.warn('Firestore CMS content read issue, using fallback/cache:', err?.message || err);
  }

  // Fallback to local default data if cache is empty
  if (!cachedContent) {
    cachedContent = readLocalFallback<CmsDatabase>('content.json', defaultCmsDatabase);
  }
  return cachedContent;
}
...
export async function saveCmsContent(content: CmsDatabase, updatedBy: string): Promise<void> {
  ...
  try {
    ...
    console.warn('Firestore CMS content save warning:', err?.message || err);
    // Write local backup only if running locally in container (non-Vercel)
    try {
...
// ==========================================
// 4. Enquiries & Commercial Submissions Operations
// ==========================================
...
export async function getEnquiries(): Promise<StoredEnquiry[]> {
  try {
    if (isUsingAdminSdk && adminDb) {
      ...
      // Check submissions collection as well
    ...
    console.warn('Firestore enquiries fetch warning:', err?.message || err);
  }

  return readLocalFallback<StoredEnquiry[]>('enquiries.json', []);
}
...
export async function saveEnquiry(enquiry: StoredEnquiry): Promise<void> {
  try {
    ...
    console.warn('Firestore enquiry save warning:', err?.message || err);
    try {
      if (!process.env.VERCEL && fs.existsSync(DATA_DIR)) {
        const existing = readLocalFallback<StoredEnquiry[]>('enquiries.json', []);
...
export async function updateEnquiryStatus(id: string, status: StoredEnquiry['status'], notes?: string): Promise<boolean> {
  ...
  try {
    ...
    console.warn('Firestore enquiry update status warning:', err?.message || err);
    return false;
  }
}
...
export async function deleteEnquiry(id: string): Promise<boolean> {
  try {
    ...
    console.warn('Firestore enquiry delete warning:', err?.message || err);
    return false;
  }
}

// ==========================================
// 5. Staff Account & Permission Operations
// ==========================================
...
export async function getUsers(): Promise<StoredUser[]> {
  try {
    ...
    console.warn('Firestore users fetch warning:', err?.message || err);
  }

  return readLocalFallback<StoredUser[]>('users.json', []);
}
...
export async function getUserById(id: string): Promise<StoredUser | null> {
  try {
    ...
    console.warn('Firestore getUserById warning:', err?.message || err);
  }

  const users = readLocalFallback<StoredUser[]>('users.json', []);
...
export async function saveUser(user: StoredUser): Promise<void> {
  try {
    ...
    console.warn('Firestore saveUser warning:', err?.message || err);
  }

  // Sync to local fallback only if outside Vercel
  try {
    if (!process.env.VERCEL && fs.existsSync(DATA_DIR)) {
      const users = readLocalFallback<StoredUser[]>('users.json', []);
...
export async function deleteUser(id: string): Promise<boolean> {
  try {
    ...
    console.warn('Firestore deleteUser warning:', err?.message || err);
  }
...
export async function getBootstrapStatus(): Promise<{ hasAdmin: boolean; adminCount: number }> {
  try {
    ...
    console.warn('Firestore bootstrap check warning:', err?.message || err);
  }
...
export async function markBootstrapped(adminUser: StoredUser): Promise<void> {
  ...
  try {
    ...
    console.warn('Firestore markBootstrapped warning:', err?.message || err);
  }
}
...
export async function getAuditLogs(): Promise<StoredAuditEntry[]> {
  try {
    ...
    console.warn('Firestore audit logs fetch warning:', err?.message || err);
  }

  return readLocalFallback<StoredAuditEntry[]>('audit.json', []);
}
...
export async function addAuditLog(entry: StoredAuditEntry): Promise<void> {
  try {
    ...
    console.warn('Firestore addAuditLog warning:', err?.message || err);
  }

  try {
    if (!process.env.VERCEL && fs.existsSync(DATA_DIR)) {
      const existing = readLocalFallback<StoredAuditEntry[]>('audit.json', []);
...
export async function uploadMedia(
  ...
  if (isUsingAdminSdk && adminStorageBucket) {
    try {
      ...
      console.warn('Admin storage upload error, falling back:', e?.message || e);
    }
  }
  ...
  try {
    ...
    console.warn('Client storage upload warning:', err?.message || err);
  }
...
export { adminDb, adminAuth, adminStorageBucket, isUsingAdminSdk };

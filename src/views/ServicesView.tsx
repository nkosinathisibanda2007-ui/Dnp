...
import { CheckCircle2, ArrowUpRight, Users, Briefcase, Award, ShieldCheck, Tractor } from 'lucide-react';
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {data.services.map((serv, index) => (
            <div
              key={serv.id}
              className="rounded-2xl bg-[#faf9f5] border border-[#ded8c4] p-6 sm:p-8 flex flex-col justify-between hover:shadow-md transition-all space-y-6"
            >
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <span className="font-mono text-xs font-bold text-[#8b6527] px-2.5 py-1 rounded bg-[#ede8d8]">
                    SERVICE 0{index + 1}
                  </span>
                  <Tractor className="w-5 h-5 text-[#8b6527]" />
                </div>
